...
pub unsafe extern "C" fn cavc_shape_create(
    ...
    ffi_catch_unwind!({
        ...
        let plinelist = unsafe { &(*plinelist).0 };
        let count : usize = plinelist.len();

        let mut v: Vec<Polyline<f64>> = Vec::with_capacity(count);

        for pline in plinelist.iter() {
            let pline = unsafe { &(**pline).0 };
            v.push(pline.clone());
        }
...
pub unsafe extern "C" fn cavc_shape_parallel_offset(
    ...
    ffi_catch_unwind!({
        ...
        let shape = unsafe { &(*shape).0 };

        let results = if options.is_null() {
            let default_options = ShapeOffsetOptions::new();
            shape.parallel_offset(offset, default_options)
        } else {
            let options = unsafe { &(*options) };
            shape.parallel_offset(offset, unsafe { options.to_internal() })
        };
...
pub unsafe extern "C" fn cavc_shape_get_ccw_polyline_count(
    ...
    ffi_catch_unwind!({
        ...
        if polyline_index as usize >= (shape.ccw_plines.len()) {
            return 2;
        }
        ...
        // prevent memory corruption/access errors but just panic as internal error if it does occur
        unsafe {
            count.write(u32::try_from(shape.ccw_plines[polyline_index as usize].polyline.vertex_data.len()).unwrap());
        }
        0
    })
}
...
pub unsafe extern "C" fn cavc_shape_get_ccw_polyline_is_closed(
    ...
    ffi_catch_unwind!({
        ...
        // prevent memory corruption/access errors but just panic as internal error if it does occur
        unsafe {
            is_closed.write(u8::try_from(shape.ccw_plines[polyline_index as usize].polyline.is_closed).unwrap());
        }
        0
    })
}
...
pub unsafe extern "C" fn cavc_shape_get_ccw_polyline_vertex_data(
    ...
    ffi_catch_unwind!({
        ...
        let pline: *const Polyline<f64> = &(shape.ccw_plines[polyline_index as usize].polyline);
        let pline = unsafe { &*pline };

        let buffer = unsafe { slice::from_raw_parts_mut(vertex_data, pline.vertex_count()) };
...
pub unsafe extern "C" fn cavc_shape_get_cw_count(
    ...
    ffi_catch_unwind!({
        ...
        // prevent memory corruption/access errors but just panic as internal error if it does occur
        unsafe {
            count.write(u32::try_from(shape.cw_plines.len()).unwrap());
        }
        0
    })
}
...
pub unsafe extern "C" fn cavc_shape_get_cw_polyline_count(
    ...
    ffi_catch_unwind!({
        ...
        // prevent memory corruption/access errors but just panic as internal error if it does occur
        unsafe {
            count.write(u32::try_from(shape.cw_plines[polyline_index as usize].polyline.vertex_data.len()).unwrap());
        }
        0
    })
}
...
pub unsafe extern "C" fn cavc_shape_get_cw_polyline_is_closed(
    ...
    ffi_catch_unwind!({
        ...
        // prevent memory corruption/access errors but just panic as internal error if it does occur
        unsafe {
            is_closed.write(u8::try_from(shape.cw_plines[polyline_index as usize].polyline.is_closed).unwrap());
        }
        0
    })
}
...
pub unsafe extern "C" fn cavc_shape_get_cw_polyline_vertex_data(
    ...
    ffi_catch_unwind!({
        ...
        let pline: *const Polyline<f64> = &(shape.cw_plines[polyline_index as usize].polyline);
        let pline = unsafe { &*pline };
